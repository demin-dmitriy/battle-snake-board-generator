  ...
  for (let y = boardState.board.height -1 ; y >= 0 ; y--) {
    ...
    for (let x = 0; x < boardState.board.width; x++) {
      ...
      if (boardState.board.food.some(food => food.x === x && food.y === y)) {
        output[boardState.board.height -1 - y].push(<CellComponent key={`${x},${boardState.board.height -1 - y}`} colour="orange" onChange={() => onChange(x, boardState.board.height -1 - y, "food")} />)
